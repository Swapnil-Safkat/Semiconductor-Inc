  ...
  return (
    <div className='w-11/12 sm:w-3/4 md:w-2/4 lg:w-1/4 my-auto'>
      <div className="w-full p-5 rounded-2xl  shadow-2xl">
        <h3 className="font-bold text-lg text-center text-cyan-300 my-6">Login</h3>
        <form onSubmit={handleSubmit(onSubmit)} className='text-white'>
          ...
          <button onClick={passwordReset} className='text-[12px] w-full text-right font-semibold text-cyan-300 mr-2  hover:cursor-pointer hover:underline'>Forgot Password?</button>
          <input disabled={loadingEmail} type="submit" value='Login' className="btn modal-action w-full hover:text-white my-2" />
          <p className='text-[12px] text-red-500 text-center font-semibold mt-4'>{errorEmail && errorEmail?.message}</p>
        </form>

        <Link to='/signup'>
          <h1 className='text-[14px] text-center font-semibold text-cyan-300 ml-2 my-2 hover:cursor-pointer hover:underline'>New to Semiconductor Inc? <span className='text-fuchsia-400'>Create new account</span></h1>
        </Link>

        <div className="divider px-4 text-base text-cyan-300 mt-4">OR</div>
